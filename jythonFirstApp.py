...
from javax.swing import (BoxLayout, ImageIcon, JButton, JFrame, JPanel, JPasswordField, JLabel, JTextArea, JTextField,JScrollPane, SwingConstants, WindowConstants)
...
class JyTwitter(object):
	def __init__(self):
		self.frame = JFrame('Jython Twitter', defaultCloseOperation = WindowConstants.EXIT_ON_CLOSE)
	...
	def timeline(self, username):
		...
		scrollpane = JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER)
	...
	def showTweet(self, status):
		...
		p.add(JTextArea(text = status.text, editable = False, wrapStyleWord = True, lineWrap = True, alignmentX = Component.LEFT_ALIGNMENT,
size = (300, 1)))
